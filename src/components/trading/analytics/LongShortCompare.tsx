    ...
    {a.longVsShort.map((s) => (
      <div key={s.side} className="rounded-xl border border-border bg-card p-3">
        ...
        <div className="space-y-1 font-mono text-xs">
          ...
          <div className="flex justify-between"><span className="text-muted-foreground">Net P&L</span><span className={`font-bold ${s.pnl >= 0 ? "text-profit" : "text-loss"}`}>{s.pnl >= 0 ? "+" : ""}{s.pnl.toFixed(2)}</span></div>
        </div>
      </div>
    ))}
  </div>
