...
import { Sparkles, CalendarDays, BookOpen, Target, ArrowRight, Flame, Goal } from "lucide-react";
...
const focusContent: Record<string, { title: string; body: string; action: string; icon: typeof Flame }> = {
  Consistency: { title: "Build your first streak", body: "Choose one small habit you can repeat today. Consistency beats intensity.", action: "Add your first habit", icon: Flame },
  Goals: { title: "Turn a goal into action", body: "Start with one habit that moves your biggest goal forward.", action: "Add a goal-supporting habit", icon: Goal },
  "Self-reflection": { title: "Make reflection part of the system", body: "Build one daily habit, then use your journal to learn what helps you keep it.", action: "Start with one habit", icon: BookOpen },
  "Trading discipline": { title: "Build trading discipline", body: "Track one repeatable process habit before you worry about the outcome.", action: "Add a trading habit", icon: Target },
};
...
const Index = ({ onboardingFocus }: IndexProps) => {
  ...
  return (
    <div className="min-h-screen bg-background"><div className="mx-auto max-w-lg px-5 py-8 pb-20">
      <motion.header className="mb-8" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="flex items-start justify-between"><div><h1 className="text-3xl text-foreground">{greeting}{firstName ? `, ${firstName}` : ""}</h1><p className="text-sm text-muted-foreground mt-1 flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" /> {dateDisplay}</p></div><ProgressRing progress={progress} size={80} strokeWidth={6} /></div>
        {progress === 1 && habits.length > 0 && <motion.div className="mt-4 flex items-center gap-2 rounded-2xl bg-primary/10 border border-primary/20 px-4 py-3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}><Sparkles className="h-4 w-4 text-primary" /><span className="text-sm font-medium text-primary">All habits complete today! 🎉</span></motion.div>}
      </motion.header>

      {showQuickStart && habits.length === 0 && <motion.section className="mb-8 rounded-2xl border border-primary/20 bg-primary/5 p-5" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10">{FocusIcon ? <FocusIcon className="h-5 w-5 text-primary" /> : <Sparkles className="h-5 w-5 text-primary" />}</div><div className="min-w-0 flex-1">
          ...
          <div className="mt-4 flex flex-wrap gap-2"><AddHabitDialog onAdd={(h) => { addHabit(h.name, h.emoji); setShowQuickStart(false); }} /><Button variant="ghost" className="rounded-xl" onClick={() => setShowQuickStart(false)}>I'll do it later</Button></div>
        </div></div>
      </motion.section>}
      ...
      <section className="mb-8"><h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Your Progress</h2><StatsCharts log={log} habits={habits.map((h) => ({ ...h, createdAt: h.created_at }))} /></section>
      <motion.section className="rounded-2xl border border-border bg-card p-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}><Heatmap log={log} habits={habits.map((h) => ({ ...h, createdAt: h.created_at }))} /></motion.section>
    </div></div>
  );
};
